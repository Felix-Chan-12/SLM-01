...
class KDTree:
    ...
    def _split_sub_tree(self, data, depth=0):
        ...
        l = depth % data.shape[1] # 没有使用方差的比较，且因为从0开始，所以是 j mod k; 而非 (j mod k)+1
        # 对数据进行排序
        data = data[data[:, l].argsort()] # 依据当前特征维度l，对data进行从小到大的排序
        # 算法3.2第1步：将所有实例坐标的中位数作为切分点
        median_index = data.shape[0] // 2
        # 获取结点在数据集中的位置 （指未排序时的位置）
    ...
    def query(self, data, k=1): # 这里的data指待分类的点
